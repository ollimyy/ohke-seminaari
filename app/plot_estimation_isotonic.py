...
def estimate_and_plot(condition: int, item_description: str):
    ...
    y_ = model.fit_transform(X.values.flatten(), y)

    # make a prediction based on the condition
    estimated_price = model.predict([condition])[0]
    ...
    plt.plot(x_range, y_range, color='red', linewidth=3, label='Isotonic Regression')
